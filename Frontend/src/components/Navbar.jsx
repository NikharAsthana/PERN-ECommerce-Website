...
const Navbar = () => {
  ...
  // tanstack provides many hooks.
  // would have to create many different states (useState) if we were not using tanstack.
  const { data: meData } = useQuery({
    queryKey: ["me"],
    queryFn: () => apiFetch("/api/me", { getToken: getToken }), // mehod is get by default
    enabled: isSignedIn, // only trigger the query if user is signed in
  });

  const role = meData?.user?.role;

  // manage cart item count
      ...
      <div className="navbar mx-auto min-h-14 max-w-7xl px-4 py-2.5 md:px-6 md:py-3">
        <div className="flex-1">
          ...
          >
            ...
            <span className="leading-none">NeoBuy</span>
          </Link>
        </div>
